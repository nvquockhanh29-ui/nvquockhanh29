          ...
          <div className="mb-3">
            ...
            {referenceImage ? (
                <div className="relative group">
                    ...
                    <button onClick={() => setReferenceImage(null)} className="absolute top-1 right-1 bg-black/60 rounded-full text-white hover:bg-black/80 p-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 z-10"><Icon name="x-circle" className="w-4 h-4" /></button>
                </div>
            ) : (
              ...
              {showMergedActions && (
                <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
                  <a 
                    ...
                    download={`huet-merged-${Date.now()}.png`} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#1e293b] p-3 rounded-xl border border-slate-700/50 aspect-video flex flex-col relative group">
                <h3 className="font-semibold text-slate-300 mb-2 text-center flex-shrink-0">{t('editorBetaFinalResult')}</h3>
                 ...
                 {!isLoading && finalResult && (
                    <div className="absolute top-2 right-2 flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        <a href={sourceImageToDataUrl(finalResult)} download={`huet-edited-${Date.now()}.png`} className="bg-slate-800/80 backdrop-blur-sm border border-slate-600 hover:bg-slate-700 text-white p-2 rounded-md" title={t('downloadImage')}>
                            <Icon name="download" className="w-5 h-5" />
                        </a>
                    </div>
                )}
